...
- [Prior Labs' TabPFN-3 documentation](https://docs.priorlabs.ai/changelog/tabpfn-3) describes tabular inference and local weights subject to licence acceptance. The optional [TabPFN-3 GPU adapter](tabpfn-comparison.md) now implements this comparison and records a measured synthetic run. It uses a pinned, separately licensed checkpoint; weights are not distributed with com-jepa.
